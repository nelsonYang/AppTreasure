 ...
 */
@ServiceConfig(
        act = AppTreasureActionNames.inquireConsumeChannelByMoney,
        importantParameters = {"money", "session", "encryptType"},
        ...
        description = "提款时查询扣除的渠道",
        validateParameters = {
    @Field(fieldName = "money", fieldType = FieldTypeEnum.BIG_INT, description = "数量"),
    @Field(fieldName = "session", fieldType = FieldTypeEnum.CHAR1024, description = "session信息"),
    @Field(fieldName = "encryptType", fieldType = FieldTypeEnum.TYINT, description = "加密类型")
})
public class InquireConsumeChannelByMoneyService implements Service {

    private Logger logger = LogFactory.getInstance().getLogger(InquireConsumeChannelByMoneyService.class);

    public void execute() {
        ApplicationContext applicationContext = ApplicationContext.CTX;
        long userId = applicationContext.getUserId();
        Map<String, String> parameters = applicationContext.getSimpleMapParameters();
        String moneyStr = parameters.get("money");
        logger.debug("parameters={}", parameters);
        long money = Long.parseLong(moneyStr);
        if (money >= AppTreasureConstant.DRAW_MIN_MONEY && money % AppTreasureConstant.DRAW_MIN_MONEY == 0) {
            ...
            if (accountList != null && !accountList.isEmpty()) {
                long productIntegeration = Long.parseLong(moneyStr);
...
//                    }
            ...
            applicationContext.fail(AppTreasureErrorCode.MONEY_NOT_FIFTY_TIMES);
        }
    }
}

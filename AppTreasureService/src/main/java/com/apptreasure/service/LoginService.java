 ...
 */
@ServiceConfig(
        act = AppTreasureActionNames.login,
    ...
    public void execute() {
        ...
        if (userList != null && !userList.isEmpty()) {
            try {
                ...
                content.append("{\"userName\":\"").append(user.getUserName())
                        .append("\",\"token\":\"").append(token).append("\",\"userId\":\"").append(user.getUserId())
                ...
                if (base64Content != null) {
                    ...
                    boolean isSuccess = applicationContext.getSessionManager().putKey(String.valueOf(user.getUserId()), sessionEntity);
                    if (isSuccess) {
                        //获取key
                        String key = UUID.randomUUID().toString();
                        String realKey = key.replace("-", "").substring(0, 16);
                        applicationContext.putKey(String.valueOf(user.getUserId()), realKey);
